# Plan: timetap — the honest record round
brief: factory/BRIEF-2.md
date: 2026-07-27
tests defined: 2026-07-27

16 tasks in four stages. Ordered so the trust spine lands first: if the night
runs out, it runs out in Stage D, and Stages A-C are already shipped and green.

This is above the 9-12 tasks round 1 established as one clean overnight run, and
that is a deliberate, recorded decision (BRIEF-2 decision 1). The stage
boundaries are what make it survivable — every stage leaves the app runnable,
deployable and coherent on its own, so an unreached stage is a named remainder
...
| Ends after | You have |
|---|---|
| A | A day with a real boundary. Guessed blocks visible as guesses in the calendar. `waking h` and `sitting %` mean something again. Guessed hours not yet separable in the sheet. |
| B | The whole trust question closed. Every hour in the sheet is traceable to something you said or something marked as the app's guess. **This is the milestone that matters.** |
| C | It stops fighting you on fast days, and a misfiled block can be fixed without opening Google Calendar. |
| D | Complete. |

Unlike round 1, this round **does** change normal-use behaviour: Stage A adds a
control and Stage C changes what a tap does inside the first minute. Every task
below says what it changes, and the existing tests for untouched behaviour must
...
- what: `?` becomes a valid mark everywhere a mark is handled. `buildTitle_`
  (`Code.gs:394`) accepts it; `parseTitle_`'s regex (`Code.gs:422`,
  `/(?:^|\s)([+=\-])$/`) recognises it; `validOp_` (`Code.gs:625`,
  `'+=-'.indexOf(op.mark)`) stops dropping ops that carry it; and the client's
  `markFor` (`Index.html:852`) never produces it. Nothing user-visible changes —
  no code path writes a `?` yet. This task exists so that A2 has somewhere to
  write it and so the widening is proved on its own, before anything depends on
  it.
- proves: Precondition for A2 and for every mark column in Stage B. The brief's
  decision 3. Contract 17, 18.
- risk: **high** (the title format is load-bearing and round 1 found two separate
  bugs in it. Four call sites default to `ADM` when parsing fails, so a regression
  here does not throw — it silently misfiles time. A mistake in this task poisons
...
- **Vacuity check required:** revert the regex to `[+=\-]` and confirm the parse
  criteria go red. A widening test that passes against the un-widened code is
  testing nothing.
...
- what: `staleGuard_` (`Code.gs:563`) currently marks a bounded block `=` when it
  ...
  a `BODY` block the app had to guess is still a guess.
- proves: "A night you forgot" from the brief's done-test. Contract 13, 14, 15.
- risk: medium (`staleGuard_` is code fix pass 2 worked in, and fix pass 2 was
  never independently reviewed — verify rather than assume. Its `UNLOGGED`
...
- test notes: `test/tests.js`, driven by `reset(atMs)` and `setNow`/`advance`
  rather than real waiting. `A()` and `S()` return the two calendars' events
  sorted by start. Run under all four contracted timezones — the day-border cap is
  where a timezone bug would hide.
- **Vacuity check required:** revert `?` to `=` in the guard and confirm the first
  three criteria go red while the boundary criteria stay green. That separation is
  the point: it proves the boundary assertions are testing the arithmetic and not
  the mark.

### A3. STOP ends the day, and opens nothing

- what: A STOP control in the posture row (`Index.html:295`-`312`). It arms on
  the first press and acts on the second, mirroring `armDead` (`Index.html:759`)
  and `arm` (`Index.html:859`). Acting closes the open block at that instant and
  closes any open SIT at the same instant, and opens nothing — `S.open` and
  `S.sit` both go null and the grid enters its existing idle state
  (`Index.html:1159`). It shows the mark strip for the closed block exactly as a
  transition does, because ending a block is a close. With nothing open it is
  visibly inert and does nothing. No new server op is needed: `closeActual`
  (`Code.gs:700`) and `closeSit` (`Code.gs:768`) already work standalone.
- proves: "Ending the day" from the brief's done-test. Contract 8, 9, 10, 11, 12.
...
- acceptance criteria:
  ...
  - [tier 1] Given a running block, when STOP is armed once and `CONFIRM_TIMEOUT_MS`
    elapses, then the queue is empty, the block is still open, and the control has
    returned to its resting label.
  ...
  - [tier 1, error] Given a running block, when STOP is confirmed twice in quick
    succession, then exactly one close is queued. A second confirm against an
    already-closed day is a no-op, not a second write.
- test notes: `test/tests.js`, using `setServerReject` for the rejecting-server
  criterion and `reboot()` for the reload one. The mark-strip criteria reuse the
  existing strip helpers. Ordering of `closeActual` versus `closeSit` in the queue
  is an implementation choice — assert that both are present and that neither is
  an open, rather than pinning an order the code has no reason to guarantee.
...
- proves: The brief's first two open risks, converted from hopes into measured
  facts.
...
- acceptance criteria:
  ...
  - [tier 1] `test/smoke.js` gains a check that the STOP control exists, is
    reachable, and carries an accessible label — so the phone paste covers it too.
...
- what: `dayStats_` computes `waking` as first-event-start to last-event-end over
  ...
  00:00 — which is why `waking h` and the `sitting %` derived from it currently
  measure nothing. Guessed and unlogged spans stop extending the waking span.
  They keep their own hours columns; only their contribution to `first` and
  `last` goes.
- proves: "`waking h` and `sitting %` mean something" from the brief's done-test.
  Contract 21, 22.
- risk: medium (this changes the meaning of an existing column. Days logged
  before this ships are not comparable with days after, and nothing in the sheet
  will say so — flagged in the brief as a review-time decision about renaming.)
- acceptance criteria:
...
- test notes: `test/tests.js`, against the existing rollup fixtures. The golden
  comparison is the load-bearing one: it is what proves this task changed the
  abnormal case and left the normal one alone. Run under all four contracted
  timezones, because every boundary here is a local-midnight boundary.
...
app's guess, and the weekly ritual has data in it for the first time.
...
- what: `dayStats_` (`Code.gs:958`) parses `p.mark` and then never reads it, so
  ...
  per-key, per-mark breakdown of hours alongside the per-key totals it already
  produces. Marks are `+`, `=`, `-`, `?` and unmarked — five buckets, and
  unmarked is a real bucket, not an omission (blocks under `MIN_MARK_MINUTES`
  legitimately carry no mark). The existing per-key totals stay exactly as they
  are: the breakdown is additional, and the totals must still equal the sum of
  their buckets.
- proves: The data layer for "The numbers" in the brief's done-test. Contract 19.
  Nothing visible in the sheet yet.
- risk: low
- acceptance criteria:
  ...
  - [tier 1, error] Given a day with no ACTUAL events, then every bucket is 0 and
    nothing throws.
- test notes: `test/tests.js`. This is pure arithmetic over the existing fixtures,
  so it needs no clock. The per-key sum criterion is the one that catches a bucket
  being double-counted or silently dropped, and it should be written as a loop over
  keys rather than as hand-picked examples.
...
- what: `dailyGrid_` (`Code.gs:995`) gains the breakdown from B1 as columns,
  **appended after every column that exists today**. Appending rather than
  ...
  one of them. `stampGrid_` (`Code.gs:1095`) puts the last-rebuilt stamp after
  the last data column, so it moves with the width — one stamp, still exactly one.
- proves: "`DW ?` holds the guessed hours; `DW =` holds the ones you closed
  yourself" from the brief's done-test. Contract 19, 20.
...
- acceptance criteria:
  ...
  - [tier 1, error] Given a retired category with logged history
    ...
    columns. A retired key keeps reporting, which is the whole reason
    `retiredKeys_` exists.
...
- **If the index assertion fails against today's unmodified code, that is a
  finding: report it and stop.** It is asserting a property the current code
  already has.

### B3. The weekly tab carries the same breakdown

- what: `weeklyGrid_` (`Code.gs:1017`) gains the same columns on the same rule —
  appended, existing positions untouched. Weekly already carries three columns
  per key (plan, actual, ratio), so it is the wider of the two tabs and the one
  where an interleaving mistake would be most expensive.
- proves: Completes "The numbers". Contract 19, 20. The weekly tab is the one
  read on a Sunday.
...
- test notes: As B2. The day-to-week summation criterion is the one that catches a
  bucket being summed into the wrong key, which the per-key totals would not
  reveal because they would still add up.
...
- what: `dayStats_` only counts a PLAN event whose title parses to a key already
  ...
  forever, with nothing anywhere saying why. The rollup counts PLAN events found
  and PLAN events parsed, keeps both in the existing rollup script property
  (`Code.gs:824`), and reports them through `setupRollup` (`Code.gs:1145`) and
  `rollupStatus` (`Code.gs:888`). Found 12, parsed 0 is stated in those words.
  It reports and stops — it never guesses what an unparsed title meant, which
  round 1 rejected for reasons that still hold (`Code.gs:941`).
- proves: "PLAN says what it read" from the brief's done-test. Contract 23.
- risk: low
- acceptance criteria:
  ...
  - [tier 1] Given a PLAN event titled `9:00 standup`, then it counts as found and
    **not** as parsed — it parses to key `9`, which is not configured. The count
    is of events that reached a category, not of events the regex matched.
  - [tier 1] Given the daily and weekly grids, then neither contains the counts.
    The grids stay pure numbers; the counts live in the editor's report.
  ...
  - [tier 1, error] Given a rollup that fails before it reads PLAN, then the
    previous run's counts are preserved rather than overwritten with zeroes —
    same rule round 1 set for `lastSuccessMs` (`Code.gs:843`).
- test notes: `test/tests.js`, seeding `CALS.plan.events` and reading
  `SCRIPT_PROPS` plus `LOGGED` (the `say_` log, which is where a person actually
  sees this). The `9:00 standup` criterion is the one that distinguishes "the
  regex matched" from "the event counted", which is the whole point of the task.
...
- what: `SETUP.md`'s PLAN section currently reads, in full, "Nothing to
  ...
  populate themselves. Nothing in the repo states that a PLAN event only counts
  if its title begins with a category key and a colon. `SETUP.md` and `README.md`
  both say it, with an example. A lint rule pins the claim, in the house style
  where every rule maps to a bug that shipped.
- proves: Completes "PLAN says what it read". Contract 24. B4 tells you the count
  is zero; this tells you what to do about it.
- risk: low
- acceptance criteria:
...
- test notes: `test/lint.js`, following the existing `check(name, bad, why)` shape.
  This is the same family as round 1's scope-count rule: a claim in prose checked
  against the source it describes.
- **Vacuity check required:** both halves of the last criterion must be
  demonstrated, one file at a time. A rule that only fires when both files change
  is a rule that will not fire.
...
- what: `CONFIRM_WITHIN_SECONDS` is 60 and `MISTAP_SECONDS` is 90
  (`Code.gs:52`, `Code.gs:61`), so between 60 and 90 seconds a tap on a different
  category acts immediately **and** destructively — it retitles the block you are
  in rather than starting a new one (`Index.html:895`-`906`). The correction
  window shrinks well inside the confirm window (20s inside 60s), which makes
  every destructive path confirmed by construction rather than by luck, and makes
  a deliberate 45-second block recordable for the first time. A lint rule pins
  `MISTAP_SECONDS < CONFIRM_WITHIN_SECONDS` so the nesting cannot silently
  regress.
- proves: "Fast switching" from the brief's done-test. The brief's decision 5.
  Contract 25, 26, 27, 28.
...
- test notes: `test/tests.js`, driven by `advance`. The sweep is a loop, and it is
  the criterion that matters most: the individual window tests would all pass
  against a build where the two constants were merely different, and only the sweep
  proves there is no reachable gap. The lint rule reads both constants out of
  `Code.gs` rather than being told their values.
...
- what: Today the armed state reads `TAP AGAIN` (`Index.html:1178`), which does
  not disclose whether confirming will retitle the block you are in or start a
  new one. Inside the correction window it reads `TAP AGAIN TO RETITLE`; outside
  it, `TAP AGAIN TO SWITCH`. This is the same rule round 1 settled on for
  `TAP AGAIN TO DISCARD`: state what the next tap will do, and stop.
- proves: Completes "Fast switching". Without this, C1 is correct and still
  ambiguous at the moment of use.
- risk: low
- acceptance criteria:
  ...
  - [tier 1, error] Given a block is armed inside the correction window and the
    ...
    agree at the moment of the tap. A stale label that promises the wrong action is
    the exact bug this task exists to prevent.
...
- what: Past the correction window the app has no way to fix a misfiled block —
  a different category starts a new one, and re-tapping the lit one opens SPLIT
  (`Index.html:1196`), which only reassigns the remainder. Three hours into a
  block you realise is wrong, the only answer is Google Calendar. The SPLIT sheet
  ...
  this correctly — `opRecategorize_` (`Code.gs:711`) is what the mis-tap
  correction uses. This is a missing client affordance, not missing behaviour.
- proves: "Fixing a whole block" from the brief's done-test. Contract 29. Closes
  the seam that round 1's drawer points at.
...
- acceptance criteria:
  ...
  - [tier 1, error] Given no block is open, then the whole-block option is not
    reachable — SPLIT is only opened from a lit block, and this must not become a
    way in when there is nothing to recategorise.
...
Finding 6 and the tier-3 findings. Three unrelated bugs of the same shape: the
app states something it does not actually know. Last because each is independent,
...
- what: Two distinct holes, same shape. First, four sites do
  ...
  display paths. Second, in `dayStats_` an ACTUAL event whose title either fails
  to parse **or** parses to a key nobody configured (`Re: the thing` → `RE`,
  `9:00 standup` → `9`) contributes nothing at all — its hours vanish from the
  rollup rather than being misfiled. Both get the same answer: a key named for
  what it is, joining `rollupKeys_` (`Code.gs:950`) the way `UNLOGGED` already
  does. A column of them is a fact; adding them to Admin, or dropping them, are
  both claims.
- proves: "A title that does not parse lands under a key named for what it is"
  from the brief's done-test. Contract 30.
- risk: medium (four call sites in different contexts — `getState`,
  `staleGuard_`, `opCloseActual_`, `opSplitActual_` — and each needs the fallback
  to make sense locally, not just globally. Also widens the rollup's key set,
  which changes grid width after B2 and B3 have already fixed the column
  contract.)
- acceptance criteria:
  ...
  - [tier 1, error] Given B2 and B3 have run, then the new key gets its full set of
    ...
    updated to match.
- test notes: `test/tests.js`. The golden fixture changes again in this task,
  which is expected and reviewable — the diff should show one key's worth of
  columns appended. The four call sites should each get their own assertion; a
  single test that only exercises one of them would pass while three stayed wrong.
...
- what: `weeklyGrid_` groups the 90-day window into Monday-to-Sunday weeks
  ...
  suffix on `week of`: that would change the cell from a date to a string and
  break sorting.
- proves: The brief's tier-3 finding on partial weeks. Contract 31. The weekly tab
  is the one read on a Sunday, so a row that lies there is expensive.
- risk: low
- acceptance criteria:
...
- what: When an `openActual` is quarantined (`Index.html:589`), the block was
  never created on the calendar — so every later op for that ref is a silent
  no-op (`findByRef_` returns null, `opCloseActual_` returns early), while the
  grid keeps showing the block lit and its clock ticking. The banner does persist,
  so it is findable; the grid is what lies. When the set-aside write is the one
  that would have created the open block, the grid stops showing it as running.
- proves: The brief's tier-3 finding on quarantined opens. The one remaining
  place where the app shows a running block it knows nothing is behind.
- risk: low
- acceptance criteria:
...
- test notes: `test/tests.js` using `setServerReject` and `advance` to drive the
  retries, plus a phase in `test/headless.js` following `checkDrawer`'s shape. The
  third criterion is the one that stops this task from over-correcting into
  "any set-aside write clears the grid", which would be a different lie.
